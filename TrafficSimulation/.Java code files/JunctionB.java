...
public class JunctionB implements Runnable, IJunction {
    //junction attributes
    ...
    public JunctionB(String name, int greenLightTime, customClock clock, Road[] entryRoads, Road[] exitRoads) {
        this.name = name;
    ...
    public void vehicleMove()//method that decide the traffic light timing
    {
           
        // When junction is "B".
        if (Objects.equals(name, "JunctionB"))
        {
            for (Road roadIn : entries)//looping int each road that enter to the junction A
            {
                ...
                while (clock.ticksCount() < end && clock.ticksCount() < StaticClockVariables.simulationTime)
                {
                    ...
                    {                   
                        ...
                        System.out.print("");
                        if ( roadIn.roads[roadIn.nextOut] != null && roadIn.carsOnRoad > 0)
                        {
                            if (roadIn.roads[roadIn.nextOut].getDestination() != null && roadIn.roads[roadIn.nextOut].getDestination().equals(StaticCarParkVariables.IndustrialEstate))//if the destination is on the west
                            {
                                for ( Road roadOut : exits)//looping through the exit roads from junction B
                                {                                    
                                    if (Objects.equals(roadOut.name, StaticRoadVariables.roadB1))
                                    {                                       
                            ...
                            else 
                            {
                                ...
                                {
                                    if (Objects.equals(roadOut.name, StaticRoadVariables.roadB3))
                                    {                                        
                                        if (roadOut.carsOnRoad < roadOut.roads.length)
                                        {
                                            if (roadIn.carsOnRoad > 0)
